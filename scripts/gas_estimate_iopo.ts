...
const investInTranches = async (contracts: Contracts, accounts: Accounts): Promise<void> => {
    ...
    console.log(
        'IO tranche token purchase gas cost:',
        await contracts.nftSec.methods.buyTokens(0, 0, FORTY_ETH).estimateGas({from: accounts.investor1}),
        'Residual tranche token purchase gas cost:',
        await contracts.nftSec.methods.buyResidual(0).estimateGas({from: accounts.investor3}),
    )
    await Promise.all([
        contracts.nftSec.methods.buyTokens(0, 0, FORTY_ETH).send({from: accounts.investor1}), // Investor 1 purchases IO tranche
        contracts.nftSec.methods.buyTokens(0, 1, FORTY_ETH).send({from: accounts.investor2}), // Investor 2 purchases PO tranche
...
const redeemTranches = async (contracts: Contracts, accounts: Accounts): Promise<void> => {
    ...
    console.log(
        ...
        await redeemResidual().estimateGas({from: accounts.investor3}),
    )
    await redeemResidual().send({from: accounts.investor3})

    const redeemTranche = () => contracts.nftSec.methods.redeemTokens(0)
    console.log(
        'IO tranche token redemption gas cost:',
        await redeemTranche().estimateGas({from: accounts.investor1})
    )
    await redeemTranche().send({from: accounts.investor1})

    console.log(
        'PO tranche token redemption gas cost:',
